...
router.post("/submitData", async (req, res) => {
    const { username, selectedEvent, time, confidence, intelligence, energy } = req.body;
    ...
    db.get(checkQuery, [username, date], (err, row) => {
        ...
        db.run(query, values, function (err) {
            ...
            res.status(200).json({ success: true, message: `${selectedEvent} data logged successfully` });
        });
    });
});
